...
public:
    ...
    boolean runSpeed();
    /// Poll the motor and step it if a step is due, implementing a constant
    /// speed as set by the most recent call to setExpectedSpeed(). You must call this as
    /// frequently as possible, but at least once per step interval,
    /// \return true if the motor was stepped.
    boolean runSpeedWithAccel();
    void setExpectedSpeed(long expectedSpeed);
    void computeNewSpeedForConstantSpeed();
...
protected:
    ...
    unsigned long  mExpectedStepInterval;
