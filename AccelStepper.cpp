...
{
    if (expectedSpeed == mExpectedSpeed)
        return;

    expectedSpeed = constrain(expectedSpeed, -_maxSpeed, _maxSpeed);
    if (expectedSpeed == 0.0)
    {
        mExpectedStepInterval = 0;
    }
    else
    {
...
float AccelStepper::expectedSpeed()
{
    return mExpectedSpeed;
}
